...
jQuery(document).ready(function (e) {
    ...
    jQuery('#searchlight').val(keyword);

    var title = $(document).prop('title');
	document.title = title.replace('Search', '');
	var baseUrl = window.location.protocol + "//" + window.location.host + "/home.asp"; 
	
	jQuery('.searchBox input[type="text"], input[name="keyword"]').val(jQuery.url().param("keyword"));
...
jQuery(document).on("click", "#search-button, a.pagging", function (e) {
    ...
    if (jQuery.url().param("keyword") !== "" && jQuery.url().param("keyword") != 'undefined') {
            ...
            success: function (data, textStatus, jqXHR) {
                if (data.records !== undefined && data.records > 0) {
                    ...
                    if (data.error !== undefined) {
                        jQuery("#products").append(jQuery("#Search_Advanced-Fail").html());
                    } else {
...
function parseFacets(facets, filter) {
    if (facets !== undefined && facets.length > 0) {
        for (var i = 0; i < facets.length; i++) {
            try {
                if (facets[i] != null && facets[i].HitList.length > 0) {
                ...
                console.log("Search engine returned NULL facet in the collection - index: " + i);
            }
        }
    }
    else {
...
function parseResult(list) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        if (list[i].Thumbnail.match(/http:/gim) === null && list[i].Thumbnail.match(/https:/gim) === null && thumb !== "") {
            thumb = "/" + thumb;
        }
        if (thumb === "") {
        ...
        if (parseInt(list[i].OnSale) != 1) {
            priceStr = priceStr.substr(0, priceStr.length - 2) + "." + priceStr.substr(priceStr.length - 2, priceStr.length);
            t = replacement(t, "ITEMPRICE", "<span class='regular-price'>$" + priceStr + "</span>");
        }
        else {
            priceStr = priceStr.substr(0, priceStr.length - 2) + "." + priceStr.substr(priceStr.length - 2, priceStr.length);

            salePriceStr = parseInt(list[i].SalePrice * 100).toString();
            salePriceStr = salePriceStr.substr(0, salePriceStr.length - 2) + "." + salePriceStr.substr(salePriceStr.length - 2, salePriceStr.length);
            t = replacement(t, "ITEMPRICE", '<del class="regular-price">$' + priceStr + '</del> <span class="sale-price">$' + salePriceStr + '</span><span class="on-sale-badge on-sale">Sale</span>');
        }
...
function advSearchResultError(jqXHR, textStatus, errorThrown) {
    if (jqXHR.status == 429) {
