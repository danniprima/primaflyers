...
(function($) {
    var SearchLight = function(input, url, options) {
        ...
        container.css({
        	position: 'absolute',
        	zIndex: '1005',
        	top: $('#searchBox input[type="text"]').outerHeight(true),
            display: 'none'
        });
        ...
        if (settings.align == 'left') {
            ...
            container.css('right', $('#searchBox').innerWidth() - (input.offset().left + input.outerWidth()));
        }
        ...
        $('#searchBox form').append(container);
        ...
        $('#searchBox').bind('mousedown.searchlight', {searchlight: this}, function(evt) {
            var searchlight = evt.data.searchlight;
    ...
    SearchLight.prototype.addResultCategory = function(name, results) {
        ...
        for (var i = 0; i < results.length; i++) {
            ...
            if (this._settings.showIcons) {
                ...
                img.src = r[2] ? r[2] : 'assets/templates/common-html5/images/spacer.gif';
                $(td_d).append(img);
            }

            $(td_d).append(r[1]);
		$(td_d).append("<br><span align=right class=search-price>" + r[3] + "</span>&nbsp;<img src='assets/templates/common-html5/images/star" + r[4] + ".png'>");
